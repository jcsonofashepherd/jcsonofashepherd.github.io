$(document).ready(_ => {
	...
	/**
	 *	Attaches information to project image thumbnails
	 */
	const setThumbs = _ => {
		for (let i = 0; i < 12; i++) {
	...
	/**
	 *	Sets hiddent text onto all project slides
	 */
	const setProjectInfo = _ => {
		for (let i = 0; i < 12; i++) {
			let info = projects[i],
					descRepo,
					firstProject;
	...
	/**
	 *	Binds a hover event listener to the image container so that the right
	 *		page section fades to black
	 */
	const setBlackoutHover = _ => {
		$(".hover-blackout").off();
	...
	/**
	 *	Binds a hover event listener to certificattion listings to expand descriptions
	 */
	const setCertificateHover = _ => {
	 ...
	 */
	const appendArrs = (pageEl, arrCount, arrType) => {
	 ...
	 */
	const animateAboutArr = (ltIdx, rtIdx) => {
		let prevLtIdx,
				prevRtIdx,
	 ...
	 */
	const setTitle = pgTitle => $("title").html(pgTitle);

	/**
	 *	Click event function that sets upcoming page position/animation-wise
	 */
	const pgClick = (pgTrans, pgStyle) => {
		animateBtns(pgTrans);
	...
	const navClick = pg => {
		...
		if (!pgEl.hasClass("active")) {
			if (pgAbout.hasClass("active")) {
				if (pg == "Project" && !isThumb) showThumbs();
				pgClick(pg, null);
			}
	 ...
	 */
	const projectClick = (idx, currIdx) => {
